...
- **Nav fetch cache**: `NavTreeService` caches l1 (siblings) and l2 (children) fetches per `(path, attempt)` — children keyed by the item's own attempt, siblings by its parent attempt — so switching attempt on the same item refetches only the children. Group trees omit attempts and keep path-only reuse.
