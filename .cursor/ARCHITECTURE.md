...
- **Theming**: Customer branding CSS is injected at deploy time as an inline `<style>` on the parent page (`customStyleFile` in the customer `build-config.yaml`, handled by AlgoreaOps). The iframe is opaque-origin so parent stylesheets cannot reach inside; `DescriptionIframeComponent` snapshots the parent `:root` CSS variables at construct time via `buildParentCssVarOverrides()` and appends them to the iframe base CSS. Custom web fonts referenced by deploy-time overrides are not loaded inside the iframe and fall back to the next available family; if the design needs the exact face inside descriptions, declare `@font-face` (or a `<link>` to the CDN) in the customer CSS or inside `descriptionBaseCss`.
