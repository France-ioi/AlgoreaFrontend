...
- **Children layout** (`display_settings.children_layout`): `List` | `Grid` | `Hide` | `TwoLevels`. `TwoLevels` asks `GET /items/{id}/children` with presence flag `show_level2_children=1` and renders each L1 child as an `h2` plus a compact grand-children card grid (`alg-chapter-children-grid`, shared with `Grid`).
