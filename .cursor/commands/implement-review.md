...
description: 'Run implement → review → fix → summary on an already-approved plan. Use when planning is done and the user wants implementation with Grok 4.6 high fast angular-code-review.'
---
...
Then proceed through the skill's steps: implement in the dev subagent (auto), review in a Grok 4.6 high fast subagent (`cursor-grok-4.6-high-fast`), fix in the same dev subagent, and write the final summary (fixed / not fixed / plan coverage / manual testing needed).
