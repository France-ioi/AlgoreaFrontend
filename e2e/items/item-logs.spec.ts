...
test('checks view link in logs on task page', async ({ page }) => {
  ...
  await Promise.all([
    page.goto('/a/6379723280369399253;p=7523720120450464843;a=0;og=123456/history/'),
    page.waitForResponse(`${apiUrl}/items/6379723280369399253/log?limit=21&watched_group_id=123456`),
  ]);
...
test('checks view link in logs on item page', async ({ page }) => {
  await initAsUsualUser(page);
  await page.goto('a/7523720120450464843;p=7528142386663912287;a=0;og=123456/history');
  await page.route(`${apiUrl}/items/7523720120450464843/log?limit=21&watched_group_id=123456`, async route => {
