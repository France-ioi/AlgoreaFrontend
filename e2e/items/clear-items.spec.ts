...
test('checks old e2e items and remove it', { tag: '@no-parallelism' }, async ({ page, itemContentPage }) => {
  ...
  const allNestedNavItemLocators = (await Promise.all((await leftNavRootItem.getByText(regExpItem).all())
  ...
  for (const nestedNavItem of allNestedNavItemLocators) {
    const navItemName = nestedNavItem.trim();
    const targetItemLocator = page.locator('p-tree').getByText(navItemName).first();
    ...
    await itemContentPage.checkToastNotification(`You have delete "${navItemName}"`);
    await itemContentPage.checksIsTitleVisible(rootItemName);
  }
});
