...
export interface paths {
  ...
  "/answers/{answer_id}": {
    /**
     * Return the answer identified by the given `answer_id`.
     * * The user should have at least 'content' access rights to the `attempts.item_id` item for `answers.attempt_id`.
  ...
  "/groups/{group_id}/group-progress": {
     ...
     *
     * For all visible children of items from the `{parent_item_id}` list, displays the result of each direct child of the given `group_id` whose type is not in (Team, User).
     *
     * Restrictions:
  ...
  "/groups/{group_id}/members": {
    /**
     * Returns a list of group members (rows from the `groups_groups` table with `parent_group_id` = `group_id` and NOW() < `groups_groups.expires_at`). Rows related to users contain basic user info (`first_name` and `last_name` are only shown for the authenticated user or if the user approved access to their personal info for some group managed by the authenticated user).
     *
  ...
  "/groups/{group_id}/team-progress": {
     ...
     *
     * For all visible children of items from the `{parent_item_id}` list, displays the result of each team among the descendants of the group.
     *
     * Restrictions:
     * * The current user should be a manager of the group (or of one of its ancestors) with `can_watch_members` set to true,
     * * The current user should have `can_watch_members` >= 'result' on each of `{parent_item_ids}` items,
  ...
  "/groups/{group_id}/user-progress": {
     ...
     *
     * For all visible children of items from the `{parent_item_id}` list, displays the result of all user self-groups among the descendants of the given group (including those in teams).
     *
  ...
  "/groups/{source_group_id}/permissions/{group_id}/{item_id}": {
     ...
     * * The current user must be a manager (with `can_grant_group_access` permission) of `{source_group_id}` which should be a parent of the `{group_id}`.
     ...
     * * The user giving the access must be a manager (with `can_grant_group_access` permission) of `{source_group_id}` which should be a parent of the `{group_id}`.
  ...
  "/items/{item_id}/log": {
    /**
     * Returns rows from `answers` having `type` = "Submission" and started/validated `results` with additional info on users and items for the participant or the `{watched_group_id}` group (only one of `{as_team_id}` and `{watched_group_id}` can be given).
     *
     ...
     *
     * If `{watched_group_id}` is given, all rows of the result are related to descendant groups of `{watched_group_id}` and items that are descendants of `{item_id}` (+ `{item_id}` itself) and visible to the current user (at least 'info' access with `can_watch` >= 'result').
     *
     * If `{watched_group_id}` is not given, all rows of the result are related to the participant group (the current user or `{as_team_id}`) and items that are descendants of `{item_id}` (+ `{item_id}` itself) and visible to the current user (at least 'info' access).
     */
...
export interface operations {
  ...
  contestListMembersAdditionalTime: {
    parameters: {
      ...
      query: {
        /** Start the page from the group next to the group with `name` = `from.name` and `id` = `from.id` (`from.id` is required when `from.name` is present) */
        "from.name"?: string;
        /** Start the page from the group next to the group with `name` = `from.name` and `id`=`from.id` (`from.name` is required when from.id is present) */
        "from.id"?: number;
  ...
  contestAdminList: {
    parameters: {
      query: {
        /** Start the page from the contest next to the contest with `title` = `from.title` and `id` = `from.id` (`from.id` is required when `from.title` is present) */
        "from.title"?: string;
        /** Start the page from the contest next to the contest with `title` = `from.title` and `id`=`from.id` (`from.title` is required when from.id is present) */
        "from.id"?: number;
  ...
  membershipsView: {
    parameters: {
      query: {
        ...
        /** Start the page from the membership next to one with `member_since` = `from.member_since` and `groups.id` = `from.id` (`from.id` is required when `from.member_since` is present) */
        "from.member_since"?: string;
        /** Start the page from the membership next to one with `member_since`=`from.member_since` and `groups.id`=`from.id` (`from.member_since` is required when from.id is present) */
        "from.id"?: number;
  ...
  managedGroupsView: {
    parameters: {
      query: {
        ...
        /** Start the page from the group next to one with `type` = `{from.type}` (depending on the `sort` parameter, some other `{from.*}` parameters may be required) */
        "from.type"?: string;
        /** Start the page from the group next to one with `name` = `{from.name}` (depending on the `sort` parameter, some other `{from.*}` parameters may be required) */
        "from.name"?: string;
        /** Start the page from the group next to one with `id`=`{from.id}` (depending on the `sort` parameter, some other `{from.*}` parameters may be required) */
        "from.id"?: number;
        /** Display the first N groups */
        limit?: number;
      };
  ...
  groupChildrenView: {
    parameters: {
      ...
      query: {
        ...
        /** Start the page from the sub-group next to the sub-group with `name` = `from.name` and `id` = `from.id` (`from.id` is required when `from.name` is present, some other 'from.*' parameters may be required too depending on the `sort`) */
        "from.name"?: string;
        /** Start the page from the sub-group next to the sub-group with `type` = `from.type` and `id` = `from.id` (`from.id` is required when `from.type` is present, some other 'from.*' parameters may be required too depending on the `sort`) */
        "from.type"?: string;
        /** Start the page from the sub-group next to the sub-group with `grade` = `from.grade` and `id` = `from.id` (`from.id` is required when `from.grade` is present, some other 'from.*' parameters may be required too depending on the `sort`) */
        "from.grade"?: string;
        /** Start the page from the sub-group next to the sub-group with `id`=`from.id` (if at least one of other 'from.*' parameters is present, `sort.id` is required) */
        "from.id"?: number;
        sort?: (
          | "name"
          | "-name"
  ...
  groupManagersView: {
    parameters: {
      ...
      query: {
        ...
        sort?: ("name" | "-name" | "id" | "-id")[];
        /** Start the page from the manager next to the manager with `groups.name` = `from.name` and `groups.id`=`from.id` (`from.id` is required when `from.name` is present) */
        "from.name"?: string;
        /** Start the page from the manager next to the manager with `groups.id`=`from.id` (depending on `sort`, `from.name` may be required when `from.id` is present) */
        "from.id"?: number;
  ...
  groupsMembersView: {
    parameters: {
      ...
      query: {
        ...
        /** Start the page from the member next to the member with `groups_groups.member_since` = `from.member_since` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.member_since"?: string;
        /** Start the page from the member next to the member with `users.login` = `from.user.login` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.user.login"?: string;
        /** Start the page from the member next to the member with `users.grade` = `from.user.grade` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.user.grade"?: number;
        /** Start the page from the member next to the member with `groups.id`=`from.id` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.id"?: number;
  ...
  groupRequestsView: {
    parameters: {
      ...
      query: {
        ...
        /** Start the page from the request/invitation next to the request/invitation with `group_membership_changes.at` = `from.at` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.at"?: string;
        /** Start the page from the request/invitation next to the request/invitation whose joining user's login is `from.joining_user.login` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.joining_user.login"?: string;
        /** Start the page from the request/invitation next to the request/invitation with `group_membership_changes.action` = `from.action`, sorted numerically. (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.action"?: string;
        /** Start the page from the request/invitation next to the request/invitation with `group_membership_changes.member_id`=`from.member_id` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
  ...
  groupTeamDescendantView: {
    parameters: {
      ...
      query: {
        /** Start the page from the team next to the team with `name` = `from.name` and `id` = `from.id` (`from.id` is required when `from.name` is present) */
        "from.name"?: string;
        /** Start the page from the team next to the team with `name`=`from.name` and `id`=`from.id` (`from.name` is required when from.id is present) */
        "from.id"?: number;
        sort?: ("name" | "-name" | "id" | "-id")[];
  ...
  groupTeamProgress: {
    ...
    responses: {
      ...
      200: {
        schema: definitions["groupTeamProgressResponseRow"][];
      };
      400: responses["badRequestResponse"];
      401: responses["unauthorizedResponse"];
      403: responses["forbiddenResponse"];
  ...
  groupUserDescendantView: {
    parameters: {
      ...
      query: {
        /** Start the page from the user next to the user with self group's `name` = `from.name` and `id` = `from.id` (`from.id` is required when `from.name` is present) */
        "from.name"?: string;
        /** Start the page from the user next to the user with self group's `name`=`from.name` and `id`=`from.id` (`from.name` is required when from.id is present) */
        "from.id"?: number;
        sort?: ("name" | "-name" | "id" | "-id")[];
        /** Display the first N teams */
        limit?: number;
  ...
  groupUserProgress: {
    parameters: {
      ...
      query: {
        parent_item_ids: number[];
        /** Start the page from the user next to the user with `groups.name` = `from.name` and `groups.id` = `from.id` (`from.id` is required when `from.name` is present) */
        "from.name"?: string;
        /** Start the page from the user next to the user with `groups.name`=`from.name` and `groups.id`=`from.id` (`from.name` is required when from.id is present) */
        "from.id"?: number;
    ...
    responses: {
      ...
      200: {
        schema: definitions["groupUserProgressResponseRow"][];
      };
      400: responses["badRequestResponse"];
      401: responses["unauthorizedResponse"];
      403: responses["forbiddenResponse"];
  ...
  groupUserRequestsView: {
    parameters: {
      query: {
        ...
        /** Start the page from the request next to the request with `group_pending_requests.at` = `from.at` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.at"?: string;
        /** Start the page from the request next to the request whose user's login is `from.user.login` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.user.login"?: string;
        /** Start the page from the request next to request with name = `from.user.login` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.group.name"?: string;
        /** Start the page from the request next to the request with `group_pending_requests.group_id`=`from.group.id` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
        "from.group.id"?: number;
        /** Start the page from the request next to the request with `group_pending_requests.member_id`=`from.user.group_id` (depending on the `sort` parameter, some other `from.*` parameters may be required) */
  ...
  answersList: {
    parameters: {
      ...
      query: {
        ...
        /** Start the page from the answer next to the answer with `created_at` = `from.created_at` and `answers.id` = `from.id` (`from.id` is required when `from.created_at` is present) */
        "from.created_at"?: string;
        /** Start the page from the answer next to the answer with `created_at`=`from.created_at` and `answers.id`=`from.id` (`from.created_at` is required when from.id is present) */
        "from.id"?: number;
  ...
  itemActivityLogForItem: {
    parameters: {
      ...
      query: {
        ...
        watched_group_id?: number;
        /** Start the page from the row next to the row with `at` = `{from.at}` (all other `from.*` parameters are required when `from.at` is present) */
        "from.at"?: string;
  ...
  officialSessionsList: {
    parameters: {
      ...
      query: {
        ...
        /** Start the page from the official session next to the official session with `groups.id` = `from.group_id` (some other 'from.*' parameters may be required depending on the `{sort}`) */
        "from.group_id"?: number;
        /** Start the page from the official session next to the official session with `groups.expected_start` = `from.expected_start` (may be '[NULL]', some other 'from.*' parameters may be required depending on the `{sort}`) */
        "from.expected_start"?: string;
        /** Start the page from the official session next to the official session with `groups.name` = `from.name` (some other 'from.*' parameters may be required depending on the `{sort}`) */
        "from.name"?: string;
  ...
  userBatchesView: {
    parameters: {
      ...
      query: {
        ...
        /** Start the page from the batch next to the batch with `user_batches.size` = `from.size` (`from.group_prefix` & `from.custom_prefix` are required when `from.size` is given) */
        "from.size"?: number;
